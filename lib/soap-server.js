var http = require('http'),
	...
	url = require('url'),
	debug = require('debug')('soap-server');
...
SoapServerException.NO_SOAP_REQUEST = 'NO_SOAP_REQUEST';
...
SoapOperation.prototype.getType = function(options){
	...
	if('function' == typeof options){
		...
		for(var member in membersObject){
			var maxOccurs = false;
			var memberObject = membersObject[member];
			if(memberObject instanceof Array) {
				memberObject = memberObject[0];
				maxOccurs = 'unbounded';
			}
			if('function' == typeof memberObject && memberObject.name){
				options.members[member] = this.getType(memberObject);
				options.members[member].maxOccurs = maxOccurs;
			}
			else{
				var type = 'string';
				if(memberObject !== null && 'undefined' != typeof memberObject)
					type = typeof memberObject;
				if(memberObject instanceof Date)
					type = 'dateTime';

				options.members[member] = {type: type, maxOccurs: maxOccurs};
			}
		}
	}

	if(options instanceof Array) {
		options = this.getType(options[0]);
...
SoapServer.prototype.castArgument = function(inputName, requestArgument, options, inArray){
	if(options.maxOccurs && !inArray) {
		if(!(requestArgument instanceof Array)) {
			requestArgument = [requestArgument];
		}
		var This = this;
		return requestArgument.map(function(argument) {
			return This.castArgument(inputName, [argument], options, true);
		});
	}
	switch(options.type){
		case 'object':
			...
			for(var requestObjectAttribute in requestArgument){
				var requestAttribute = (options.members && options.members[requestObjectAttribute] && options.members[requestObjectAttribute].maxOccurs && requestArgument[requestObjectAttribute]) || requestArgument[requestObjectAttribute][0]._||requestArgument[requestObjectAttribute][0];
		...
		case 'dateTime':
			return new Date(requestArgument);
		default:
...
SoapServer.prototype.handleOperationRequest = function(service, soapRequest){
	...
	return new Promise(function(resolve, reject) {
		...
		for(var i in soapRequest) {
			if(i.substr(i.indexOf(':') + 1) == 'Envelope') {
				for(var j in soapRequest[i]) {
					if(j.substr(j.indexOf(':')+1) == 'Body') {
						operationRequest = soapRequest[i][j][0];
						break;
					}
				}
				break;
		...
		for(var operationRequestName in operationRequest){
			...
			*/
			Promise.resolve(operation.exec.apply(operation, operationArguments)).then(function(operationResponse) {
				var response = {};
...
SoapServer.prototype.js2xml = function(js, namespace, indentLevel){
	...
	var This = this;
	var doNode = function(node, element) {
		if(node == null) {
			ret += indent + '<'+ element + '/>';
		}
		else if(node instanceof Date) {
			ret += indent + '<' + element + '>' + node.toISOString() + '</' + element + '>\n';
		}
		else if('object' == typeof node){
			ret += indent + '<' + element + '>\n';
			ret += This.js2xml(node, namespace, indentLevel + 1);
			ret += indent + '</' + element + '>\n';
		}
		else{
			ret += indent + '<' + element + '>' + node + '</' + element + '>\n';
		}
	};
	...
	for(var node in js){
		...
		if(js[node] instanceof Array) {
			js[node].forEach(function(node) {
				doNode(node, element)
			});
		}
		else
			doNode(js[node], element);
	}
	return ret;
};
...
SoapServer.prototype.handleRequest = function(req, res){
	this.handleSoapRequest(req, function(exception, response){
		if(exception) {
			if(exception.name == 'SoapServerException'){
				if(exception.type == SoapServerException.SERVICE_NOT_FOUND || exception.type == SoapServerException.MISSING_REQUEST || exception.type == SoapServerException.NO_SOAP_REQUEST){
...
SoapServer.prototype.handleSoapRequest = function(req, callback){
	...
	var requestUrl = url.parse(req.originalUrl || req.url, true);
	var This = this;

	var serviceName = requestUrl.pathname.slice(requestUrl.pathname.lastIndexOf('/')+1);//req.url.substr(1);
	/*
	...
	*/
	...
	if('undefined' == typeof service)
		return callback(new SoapServerException(SoapServerException.SERVICE_NOT_FOUND, 'Service [' + serviceName + '] not found'));
	...
	if (req.method == 'POST') {
		...
		req.on('end', function () {
			...
			parseXml(body, function (err, soapRequest) {
				...
				var responseXmlHead = '<?xml version="1.0" encoding="UTF-8"?>\n';
				responseXmlHead += '<soap:Envelope\n';
				responseXmlHead += '	xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"\n';
				responseXmlHead += '	soap:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">\n';
				responseXmlHead += '	<soap:Body>\n';

				var responseXmlFoot = '	</soap:Body>\n';
				responseXmlFoot += '</soap:Envelope>';

				This.handleOperationRequest(service, soapRequest).then(function(soapResponse) {
					var responseXml = This.js2xml(soapResponse, null, 2);
					callback(null, responseXmlHead+responseXml+responseXmlFoot);
				}, function(exception) {
					if(exception.name != 'SoapServiceException')
						return callback(exception);

					var responseXml = '	<soap:Fault>\n';
	...
	if(!requestUrl.query)
		return callback(new SoapServerException(SoapServerException.MISSING_REQUEST, 'No query string supplied'));

	if(requestUrl.query.hasOwnProperty('wsdl')){
		var wsdl = this.getWsdl(req.headers.host+requestUrl.pathname, service);
		return callback(null, wsdl);
	}
	callback(new SoapServerException(SoapServerException.NO_SOAP_REQUEST, 'Current request is not a valid SOAP request'));
};
...
SoapServer.prototype.getXsdElement = function(name, options, indentLevel){
	...
	if(options.type != 'object')
		return indent + '<xsd:element name="' + name + '" type="xsd:' + this.getType(options) + '" minOccurs="0" '+(options.maxOccurs?'maxOccurs="'+options.maxOccurs+'" ':'')+'/>\n';

	var ret = indent + '<xsd:element name="' + name + '"  minOccurs="0" '+(options.maxOccurs?'maxOccurs="'+options.maxOccurs+'" ':'')+'>\n';
...
SoapServer.prototype.getWsdl = function(host, service){
	...
	for(var objectTypeName in objects){
		...
		wsdl += '			<xsd:element name="' + objectTypeName + 'List" type="tns:' + objectTypeName + '" maxOccurs="unbounded"/>\n';
	...
	for(operationName in service.operations){
		...
		for(var inputName in operation.inputs){
			var input = operation.inputs[inputName];
			if(input.type == 'object'){
				wsdl += '		<part name="' + inputName + '" element="tns:' + this.getType(input) + (input.maxOccurs?'List':'')+'"/>\n';
			}
			else{
		...
		if(operation.output.type == 'object'){
			wsdl += '		<part name="return" element="tns:' + this.getType(operation.output) + (operation.output.maxOccurs?'List':'')+'"/>\n';
		}
		else{
	...
	wsdl += '			<soap:address location="http://' + host + '" />\n';
	wsdl += '		</port>\n';
	wsdl += '	</service>\n';
	wsdl += '</definitions>';

...
SoapOperation.prototype.getType = function(options){
	...
	if('function' == typeof options){
		...
		for(var member in membersObject){
			var maxOccurs = false;
			if(membersObject[member] instanceof Array) {
				membersObject[member] = membersObject[member][0];
				maxOccurs = 'unbounded';
			}
			if('function' == typeof membersObject[member] && membersObject[member].name){
				...
				var type = 'string';
				if(membersObject[member] !== null && 'undefined' != typeof membersObject[member])
					type = typeof membersObject[member];
				if(membersObject[member] instanceof Date)
					type = 'dateTime';
	...
	switch(options.type){
		case 'object':
			...
			for(var requestObjectAttribute in requestArgument){
				var requestAttribute = requestArgument[requestObjectAttribute][0]._||requestArgument[requestObjectAttribute][0];
		...
		case 'dateTime':
			return Date.parse(requestArgument);
		default:
...
SoapServer.prototype.getWsdl = function(host, service){
	...
	for(operationName in service.operations){
		...
		for(var inputName in operation.inputs){
			var input = operation.inputs[inputName];
			if(input.type == 'object'){
				wsdl += '		<part name="' + inputName + '" element="tns:' + this.getType(input) + '" '+(input.maxOccurs?'maxOccurs="'+input.maxOccurs+'" ':'')+'/>\n';
			}
			else{
				wsdl += '		<part name="' + inputName + '" element="xsd:' + this.getType(input) + '" '+(input.maxOccurs?'maxOccurs="'+input.maxOccurs+'" ':'')+'/>\n';
			}
		}
		wsdl += '	</message>\n';
		...
		if(operation.output.type == 'object'){
			wsdl += '		<part name="return" element="tns:' + this.getType(operation.output) + '" '+(operation.output.maxOccurs?'maxOccurs="'+operation.output.maxOccurs+'" ':'')+'/>\n';
		}
		else{
			wsdl += '		<part name="return" element="xsd:' + this.getType(operation.output) + '" '+(operation.output.maxOccurs?'maxOccurs="'+operation.output.maxOccurs+'" ':'')+'/>\n';
		}
		wsdl += '	</message>\n';
